...
```powershell
    docker buildx build --tag juergenzimmermann/kunde:2023.1.0-dockerfile .
```
...
```powershell
    docker run --publish 8080:8080 `
      ...
      --name kunde --rm juergenzimmermann/kunde:2023.1.0-dockerfile
```
...
```powershell
    docker inspect juergenzimmermann/kunde:2023.1.0
```
...
```PowerSkell
    pack inspect juergenzimmermann/kunde:2023.1.0
```
...
```powershell
    cd \Zimmermann\dive
    .\dive juergenzimmermann/kunde:2023.1.0
```
...
```powershell
    docker save juergenzimmermann/kunde:2023.1.0 > kunde.tar
```
...
Um das Image mit dem Tag `2023.1.0` zu bauen, muss die Umgebungsvariable `TAG` auf
den Wert `2023.1.0` gesetzt sein. Dabei ist auf die Großschreibung bei der
...
Zunächst startet man den eigenen zu testenden Server mit dem Kommando
`gradle bootRun -Ddb=h2 -DAPPLICATION_LOGLEVEL=INFO -DREQUEST_RESPONSE_MAPPING_LOGLEVEL=INFO -DHIBERNATE_LOGLEVEL=INFO -DFLYWAY_LOGLEVEL=INFO`,
damit die Logausgaben für die Entwickler/innen ausgeblendet werden. Danach kann man den Fortio-Server
