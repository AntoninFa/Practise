...
class KundeWriteController {
    ...
    @SuppressWarnings("TrailingComment")
    ResponseEntity<Void> create(@RequestBody final KundeDTO kundeDTO, final HttpServletRequest request) {
        log.debug("create: {}", kundeDTO);

        final var kunde = service.create(kundeDTO.toKunde());
        final var baseUri = uriHelper.getBaseUri(request).toString();
        final var location = URI.create(baseUri + '/' + kunde.getId()); //NOSONAR
