 ...
 * <img src="../../../../../asciidoc/AutoWriteController.svg" alt="Klassendiagramm">
 *
...
@Slf4j
@SuppressWarnings({"ClassFanOutComplexity", "MethodCount", "DuplicatedCode", "LineLength", "TrailingComment", "RequireEmptyLineBeforeBlockTagGroup", "JavadocVariable"})
public class AutoWriteController {
     ...
     *      sind oder die FIN bereits existiert oder Statuscode 400 falls syntaktische Fehler im Request-Body
     *      vorliegen.
     * @throws URISyntaxException falls die URI im Request-Objekt nicht korrekt wäre
     */
    @PostMapping(consumes = APPLICATION_JSON_VALUE)
    @Operation(summary = "Ein neues Auto anlegen", tags = "Neuanlegen")
    @ApiResponse(responseCode = "201", description = "Auto neu angelegt")
    @ApiResponse(responseCode = "400", description = "Syntaktische Fehler im Request-Body")
    @ApiResponse(responseCode = "422", description = "Ungültige Werte oder FIN vorhanden")
    @SuppressWarnings("TrailingComment")
    ResponseEntity<Void> post(
        @RequestBody final AutoDTO autoDTO,
        final HttpServletRequest request
    ) throws URISyntaxException {
        log.debug("post: {}", autoDTO);

        if (autoDTO == null) {
            return badRequest().build();
        }

        final var auto = service.create(autoDTO.toAuto());
        final var baseUri = uriHelper.getBaseUri(request).toString();
        final var location = URI.create(baseUri + '/' + auto.getId()); //NOSONAR
     ...
     *      oder 422 falls Constraints verletzt sind oder die FIN bereits existiert
     *      oder 412 falls die Versionsnummer nicht ok ist oder 428 falls die Versionsnummer fehlt.
     */
    @PutMapping(path = "{id:" + ID_PATTERN + "}", consumes = APPLICATION_JSON_VALUE)
    @ResponseStatus(NO_CONTENT)
    @Operation(summary = "Ein Auto mit neuen Werten aktualisieren", tags = "Aktualisieren")
    @ApiResponse(responseCode = "204", description = "Aktualisiert")
    @ApiResponse(responseCode = "400", description = "Syntaktische Fehler im Request-Body")
    @ApiResponse(responseCode = "404", description = "Auto nicht vorhanden")
    ...
    ResponseEntity<Void> put(
        ...
        log.debug("put: id={}, {}", id, autoDTO);
        final int versionInt = getVersion(version, request);
        final var auto = service.update(autoDTO.toAuto(), id, versionInt);
        log.debug("put: {}", auto);
        return noContent().eTag("\"" + auto.getVersion() + '"').build();
    }
    ...
    ProblemDetail onConstraintViolations(
        ...
        final var autoViolations = ex.getViolations()
            .stream()
            .map(violation -> violation.getPropertyPath() + ": " +
                violation.getConstraintDescriptor().getAnnotation().annotationType().getSimpleName() + " " +
                violation.getMessage())
            .toList();
        log.trace("onConstraintViolations: {}", autoViolations);
        final String detail;
        if (autoViolations.isEmpty()) {
            detail = "N/A";
        } else {
            // [ und ] aus dem String der Liste entfernen
            final var violationsStr = autoViolations.toString();
    ...
    @ExceptionHandler
    ProblemDetail onFinExists(final FinExistsException ex, final HttpServletRequest request) {
        log.debug("onFinExists: {}", ex.getMessage());
